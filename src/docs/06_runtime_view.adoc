...
==== Cheching notifications

In order to check for new notifications is quite simple:

. First the user needs to click on the bell simbol of the navbar.
. Now, all the notifications will load, so the user can check them
. Once he finish, the user can click on "Accept all", so all the notifications will be deleted
...
. Finally, the applicaiton will refresh with the correct langugae.
...
. First click in "Themes" in the navbar.
